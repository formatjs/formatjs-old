...
_Note: This `IntlRelativeTimeFormat` API may change to stay in sync with ECMA-402, but this package will follow [semver][]._
...
```js
var rf = new IntlRelativeTimeFormat(locales, [options]);
```
...
The most common way to use this library is to construct an `IntlRelativeTimeFormat` instance and reuse it many times for formatting different date values; e.g.:

```js
var rf = new IntlRelativeTimeFormat('en-US');
...
* Optimized for repeated calls to an `IntlRelativeTimeFormat` instance's `format()` method.
...
### Loading IntlRelativeTimeFormat in Node.js

Install package and polyfill:

```bash
npm install intl-relativetimeformat --save
...
var IntlRelativeTimeFormat = require('intl-relativetimeformat');
var rf = new IntlRelativeTimeFormat('en');
var output = rf.format(dateValue);
```

_Note: in Node.js, the data for all 200+ languages is loaded along with the library._

### Loading IntlRelativeTimeFormat in a browser
...
```html
<script src="intl-relativetimeformat/dist/intl-relativetimeformat.min.js"></script>
```
...
```html
<script src="intl-relativetimeformat/dist/locale-data/fr.js"></script>
```
...
### Bundling IntlRelativeTimeFormat with Browserify/Webpack
...
```js
var IntlRelativeTimeFormat = require('intl-relativetimeformat');
require('intl-relativetimeformat/dist/locale-data/en.js');
require('intl-relativetimeformat/dist/locale-data/fr.js');
```
...
#### `IntlRelativeTimeFormat` Constructor

To format a date to relative time, use the `IntlRelativeTimeFormat` constructor. The constructor takes two parameters:
...
`IntlRelativeTimeFormat` uses a locale resolution process similar to that of the built-in `Intl` APIs to determine which locale data to use based on the `locales` value passed to the constructor. The result of this resolution process can be determined by call the `resolvedOptions()` prototype method.

The following are the abstract steps `IntlRelativeTimeFormat` goes through to resolve the locale value:
