...
export default function generateFieldExtractorFn<DataType extends Record<string, any>>(
    ...
    function getFields(locale: Locale) {
      ...
      if (checkCachedFieldsFn(locale)) {
        cachedFields = fields[locale] = loadFieldsFn(locale);
        return cachedFields;
      }
    }
    ...
    return locales.reduce(
      (
        fields: Record<Locale, {fields: DataType}>,
        locale
      ) => {
