...
export default class UnifiedNumberFormat {
  ...
  constructor(locale?: string, {style, unit, unitDisplay, ...options}: UnifiedNumberFormatOptions = {}) {
    ...
    this.nf = new Intl.NumberFormat(locale, {...options, style: style === 'unit' ? 'decimal': style })
  }
  ...
  format(num: number) {
    ...
    if (this.unit) {
      ...
      const patternData = UnifiedNumberFormat.__unitLocaleData__[this.unit][locale];
  ...
  static supportedLocalesOf(...args: Parameters<typeof Intl.NumberFormat.supportedLocalesOf>) {
  ...
  static __addUnitLocaleData(unit: Unit, ...data: LocaleData[]) {
    ...
    for (const datum of data) {
      ...
      UnifiedNumberFormat.__unitLocaleData__[unit][datum.locale.toLowerCase()] = datum;
    }
  }
}
